...
N <- dim(datos_python)[1] # number of fields
size_field <- dim(datos_python)[2]

cutoff <- size_field * sqrt(2) / 3 # default value in variogram()
...
for (i in 1:N){
  Z <- datos_python[i,,]
  field_xyz$Z <- rev(c(apply(Z, 1, rev))) # vectorize in the correct order
  ...
  maps_vario[i,,] <- matrix(variograma$map$var1, nrow=13, byrow=TRUE) # Save map as matrix
}
)
